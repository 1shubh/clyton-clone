...
export const SidewallDimensions = ({
  data,
  setData,
  currentForm,
  setCurrentForm,
  ...
  const handleAddSlidingPrice = (i) => {
    setData((prevData) => {
      const updatedOptions = prevData.sidewallDimensions.options.map(
        (option, index) =>
          index === i
            ? {
                ...option,
    ...
    if (newOption.title && newOption.price) {
      setData((prevData) => ({
        ...
        sidewallDimensions: {
          ...prevData.sidewallDimensions,
          options: [
            ...prevData.sidewallDimensions.options,
            {
  ...
  const handleDeleteOption = (i) => {
    setData((prevData) => {
      const updatedOptions = prevData.sidewallDimensions.options.filter(
  ...
  return (
    <div
      className={`${currentForm === "sidewallDimensions" ? "block" : "hidden"}`}
    >
      <div>
        <p className="text-xl font-bold">Add Sidewall Dimensions Options</p>
        <div className="grid gap-2 mt-5">
          {data.sidewallDimensions.options.map((ele, i) => (
            <div
              key={i}
        ...
        <div className="flex items-center justify-end">
          <Button
            ...
            onClick={() => setCurrentForm("insulationOptions")}
          >
            Save Sidewall Dimensions
          </Button>
        </div>
      </div>
    </div>
