  ...
  handleKitchenSubmit
    ...
    const handleAddBacksplashTile = (e) => {
      ...
      setKitchen((prevKitchen) => ({
        ...prevKitchen,
        kitchenSinks: {
          ...prevKitchen.kitchenSinks,
          options: [
            ...(prevKitchen.kitchenSinks?.options || []), // Ensure options is not undefined
    ...
    const handleEditPrice = (index) => {
      ...
      setEditPrice(kitchen.kitchenSinks.options[index].price);
    };
  
    // Save edited price
    const handleSavePrice = (index) => {
      setKitchen((prevKitchen) => {
        const updatedOptions = [...prevKitchen.kitchenSinks.options];
    ...
    const handleDeleteItem = async (index) => {
      const faucetToDelete = kitchen.kitchenSinks.options[index];
      ...
      setKitchen((prevKitchen) => ({
        ...
        kitchenSinks: {
          ...prevKitchen.kitchenSinks,
          options: prevKitchen.kitchenSinks.options.filter(
            (_, i) => i !== index
          ),
        },
      }));
    ...
    const isFormDisabled = kitchen.kitchenSinks.options.length < 1;
  return (
    <div
    className={`${currentForm === "kitchenSink" ? "block" : "hidden"}`}
  >
    <Button
    ...
    <p className="text-xl font-bold text-nowrap my-5">Add Kitchen Sinks</p>
    <div className="grid grid-cols-4 gap-2 pb-4 mt-5">
      {kitchen?.kitchenSinks?.options?.map((ele, i) => (
        <div
          key={i}
