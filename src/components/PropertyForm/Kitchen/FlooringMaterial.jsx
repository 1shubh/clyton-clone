...
export const FlooringMaterial = ({
  ...
  const handleCounterTopSubmit = (e) => {
    e.preventDefault();
    setCurrentForm("kitchenFaucets");
  };

  const handleCounterTopPriceChange = (i, value) => {
    setflooringMaterialPrices((prev) => ({
      ...prev,
      [i]: value,
    }));
  };
  ...
  const handleAddCounterTopPrice = (i) => {
    setKitchen((prevKitchen) => {
      const updatedOptions = prevKitchen.flooringMaterial.options.map(
        (option, index) =>
          index === i
            ? {
                ...option,
                price:
                  flooringMaterialPrices[i] !== undefined
                    ? parseFloat(flooringMaterialPrices[i])
                    : option.price,
              }
            : option
      );
  ...
  const handleDeleteType = async (optionIndex, typeIndex) => {
    const typeToDelete =
      kitchen.flooringMaterial.options[optionIndex].types[typeIndex];
  ...
  return (
    <div
      className={`${currentForm === "flooringMaterial" ? "block" : "hidden"}`}
    >
      <div className="mt-5">
        ...
        <Button
          ...
          onClick={() => handleBack()}
        >
          Back to Backsplash Tiles
        </Button>
          <p className="text-xl font-bold text-nowrap mt-5">
            Add Kitchen & bathroom flooring material
          </p>
          <div className="grid gap-2 mt-5">
            {kitchen?.flooringMaterial?.options?.map((ele, i) => (
